...
public class Data {
	...
	private static WeakHashMap<String, Video> videos = new WeakHashMap<String, Video>();
  private Data() {}
  /**
  ...
  static public Video newVideo(String title, int year, String director) {
    // TODO
	  ...
	  if (director == null || director.trim().equals("")) { throw new IllegalArgumentException(); }
	  if (year >= 5000 || year <= 1800) { throw new IllegalArgumentException(); }
	  
	  String strNotation = (title.trim() + " (" + year + ") : " + director.trim()).intern();
	  
	  if (videos.containsKey(strNotation))
	  {
	  }
	  else
	  {
		  videos.put(strNotation, new VideoObj(title.trim(), year, director.trim()));
	  }
	  return videos.get(strNotation);
